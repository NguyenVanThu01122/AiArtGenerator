...
function AiBackgroundRemove() {
  ...
  const { handleUploadImage, fileUpload, setFileUpload, uploadImage, setUploadImage } = useUploadFile()
    ...
    removeBackground(formData)
      .then(async (res) => {
        ...
        await deductCreditsRemoveBackground() // gọi api Trừ credits với các lần sử dụng
          ...
          .catch(() => toast.error(t(ERROR_MESSAGES.CREDIT_DEDUCTION_FAILED)))
      })
      .catch((error) => {
        setLoading(false)
                ...
                {fileUpload ? (
                  <Uploaded>
                    ...
                    <BoxChange loading={loading}>
                      <ImageGeneral className='icon-cancel' onClick={() => handleBack()} src={iconCancel} />
                      <ChangeItem>
                        <TextChange>
                  ...
                  <NotUploaded>
                    ...
                    <InputFile name='file' type='file' accept={FILE_FORMAT.join(',')} onChange={handleUploadImage} />
                  </NotUploaded>
                )}
              </SectionUploadImage>
              <StyledButton
