...
function AiImageResult() {
  ...
  const { listResultImg, loading, total, imageAi } = useSelector((state: RootState) => state.getResultAiImgSlice)

  const [indexStart, indexEnd, totalPage] = useMemo(() => calculateIndexes(page, limit, total), [page, limit, total])
  ...
  const handleChangeLimit = useCallback((value: number) => {
    setLimit(value)
    setPage(1)
  }, [])
  ...
  const clickCheckbox = useCallback(
    (id: string) => {
      if (!listId.includes(id)) {
        setListId((prevListId: string[]) => [...prevListId, id])
      } else {
        setListId((prevListId: string[]) => prevListId.filter((item: string) => item !== id))
      }
    },
    [listId]
  )
      ...
      {listResultImg?.length > 0 && (
        <ContainerImageAi>
          ...
          <SectionPagination sx={{ bgcolor: 'background.default' }}>
            ...
            />
            <DisplayTotalImage>
              {t('Displayed from')} {indexStart} {t('to')} {indexEnd} {t('out of')} {total} {t('photos')}
              <SelectGeneral
                value={limit}
